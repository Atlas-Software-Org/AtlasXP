...
uintptr_t VIRT_TO_PHYS(uintptr_t vaddr, uint64_t RamSize) {
    uint64_t hhdm_phys_base = RamSize - (2UL * 1024 * 1024 * 1024);
    return vaddr - HHDM_VIRT_BASE + hhdm_phys_base;
}

uintptr_t PHYS_TO_VIRT(uintptr_t paddr, uint64_t RamSize) {
    uint64_t hhdm_phys_base = RamSize - (2UL * 1024 * 1024 * 1024);
    return paddr - hhdm_phys_base + HHDM_VIRT_BASE;
}
...
static inline PageTable* KiGetOrCreateTable(uint64_t entry, bool* created) {
    if (!(entry & PAGE_PRESENT)) {
        ...
        return (PageTable*)PHYS_TO_VIRT(new_page, RamSize);
    }
    return (PageTable*)PHYS_TO_VIRT(entry & PAGE_ADDR_MASK, RamSize);
}

void KiMMap(void* vaddr, void* paddr, uint64_t attributes) {
    uint64_t cr3;
    asm volatile ("mov %%cr3, %0" : "=r"(cr3));
    PageTable* pml4 = (PageTable*)PHYS_TO_VIRT(cr3 & PAGE_ADDR_MASK, RamSize);
    ...
    if (created) pml4->Entries[pml4_index] = VIRT_TO_PHYS(pdpt, RamSize) | PAGE_PRESENT | PAGE_RW;

    PageTable* pd = KiGetOrCreateTable(pdpt->Entries[pdpt_index], &created);
    if (created) pdpt->Entries[pdpt_index] = VIRT_TO_PHYS(pd, RamSize) | PAGE_PRESENT | PAGE_RW;

    PageTable* pt = KiGetOrCreateTable(pd->Entries[pd_index], &created);
    if (created) pd->Entries[pd_index] = VIRT_TO_PHYS(pt, RamSize) | PAGE_PRESENT | PAGE_RW;
...
void KiUMap(void* virt_addr) {
    ...
    PageTable* pdpt = (PageTable*)PHYS_TO_VIRT(pml4->Entries[pml4_index] & PAGE_ADDR_MASK, RamSize);
    if (!(pml4->Entries[pml4_index] & PAGE_PRESENT)) return;

    PageTable* pd = (PageTable*)PHYS_TO_VIRT(pdpt->Entries[pdpt_index] & PAGE_ADDR_MASK, RamSize);
    if (!(pdpt->Entries[pdpt_index] & PAGE_PRESENT)) return;

    PageTable* pt = (PageTable*)PHYS_TO_VIRT(pd->Entries[pd_index] & PAGE_ADDR_MASK, RamSize);
