...
Axf64_Mhdr *ParseAXF64Executable(void* data_start) {
    Axf64_Mhdr* axf64_mhdr = data_start;

    if (axf64_mhdr->a_ident[AI_MAG0] == AIMAG0 &&
    ...
    if (axf64_mhdr->a_mclass == AI_Class_M64) {}
    else {
        return NULL;
    }
...
Axf32_Mhdr *ParseAXF32Executable(void* data_start) {
    Axf32_Mhdr* axf32_mhdr = data_start;

    if (axf32_mhdr->a_ident[AI_MAG0] == AIMAG0 &&
    ...
    if (axf32_mhdr->a_mclass == AI_Class_M32) {}
    else {
        return NULL;
    }
...
int GetAXF_MClass(void* data_start) {
    ...
    return axf32_mhdr->a_mclass;
}

typedef void (*func_t)(void);  // Define function pointer type
